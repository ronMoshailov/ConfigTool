...
public class PhaseE extends Stage {
	private Tk1 node;
	 ...
	 */
	public PhaseE(Tk1 node, String name, int number, int length, boolean isStopInPolice, Move[] sgs) {
	 ...
	 * @param sgs - list of moves that must open in this stage
	 */
	public PhaseE(Tk1 node, String name, int numberr, int length, int sp, boolean isStopInPolice, Move[] sgs) {
	...
	public Phase phasenFunktion() {
		...
		if (getPhasenZeit() >= node.lenPhue + getPhLenMS())
		{
			...
			if (isStageDoneFlag) 
			{
				return startNextPhase(node.PhueE_A);
			}
		}
