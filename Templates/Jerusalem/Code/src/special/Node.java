...
abstract public class Node extends TeilKnoten {
	...
	public TPDetector[] pushButtons; 			// pedestrian push buttons
	public Input[] inputs;						// regular inputs (pulses and etc.)
	public DDetector[] demands;				// demand detectors
	public EDetector[] extensions;				// extension detectors
	public DEDetector[] demandExtensions;		// combined demand and extension detectors
	public QDetector[] queues;					// queue detectors
	
	public Output[] outputs;					// outputs/pulses
	
	private Detektor[] detsTmp;					// auxiliary array
	private Ausgang[] ausTmp;					// auxiliary array				
	private int detIterator;					// auxiliary iterator
	
	public boolean isDetectorsError;
	public boolean isSwitchingToFire;
	public boolean isInFire;

	public int activeStageNumber = -1;
	...
	public int centralProgram = -1;
	public int clockProgram = -1;
	...
	public void UpdateDetectors() {
		...
		if (ausTmp != null) {
			for (detIterator = 0; detIterator < ausTmp.length; detIterator++) {
				if (ausTmp[detIterator].getTk().getNummer() == getNummer()) {
					if (ausTmp[detIterator] instanceof Output) {
						outputsCount++;
					}
				}
			}
		...
		if (ausTmp != null) {
			for (detIterator = 0; detIterator < ausTmp.length; detIterator++) {
				if (ausTmp[detIterator].getTk().getNummer() == getNummer()) {
					if (ausTmp[detIterator] instanceof Output) {
						outputs[outputsCount++] = (Output)ausTmp[detIterator];
					}
				}
			}
		}
	...
	private boolean isPulseReceived(int cycle, int offset) 
	{
		return ((getSecondsFromMidnight() % cycle) == offset);
	}
	...
	public boolean isTransEmClockGreenWaveSynced() {
		cycle = ((ParametersJerusalem)Var.controller.dvi35Parameters).getCycle();
		offset = ((ParametersJerusalem)Var.controller.dvi35Parameters).getOffset();
		...
		isSynced = cycle == 0 || isPulseReceived(cycle, offset);
	...
	{
		return getAktProg().getUmlaufZeit() != 900;
	}
