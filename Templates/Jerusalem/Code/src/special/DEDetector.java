...
public class DEDetector extends Input {
	...
	public DEDetector(String name, Move move, boolean isUseMapInput, boolean isCallAllable, boolean isSetFromParamsEnabled, int demandId, int gapId) {
		super(name, 0, 10, 0, move, 4000, isCallAllable, isSetFromParamsEnabled, demandId);
		...
		if (Var.controller.getAppType().equals(AppType.Haifa)) {
			...
			if (!Var.controller.isPreemption()) {
				if (detectorId > Var.controller.getMaximalDetectorId()) {
					...
					return ((ParametersJerusalem)Var.controller.dvi35Parameters).getDetectors(node, demandId - 1) == Var.INPUT_FIXED && this.isSetFromParamsEnabled;
				}
			} else {
				return false;
			}
	...
	public void UpdateState() {
		gapInt = getGap();
		setOnFromParams = ((ParametersJerusalem)Var.controller.dvi35Parameters).getDetectors(node, gapId - 1) == Var.INPUT_FIXED && this.isSetFromParamsEnabled;
		setFromParametersDemandOnly = getSetFromParametersDemandOnly();
		setFromCC = getSetFromCC();

		currentState = (!dynLuecke(gapInt) || (mapInput != null && !mapInput.dynLuecke(gapInt)) || setOnFromParams || setFromCC) && (!node.isOffline() && !(((ParametersJerusalem)Var.controller.dvi35Parameters).getDetectors(node, gapId - 1) == Var.INPUT_DISABLED && this.isSetFromParamsEnabled));
		demandOnly = (belegt() || (mapInput != null && mapInput.belegt()) || setFromParametersDemandOnly || setFromCC) && (!node.isOffline() && !(((ParametersJerusalem)Var.controller.dvi35Parameters).getDetectors(node, demandId - 1) == Var.INPUT_DISABLED && this.isSetFromParamsEnabled));
