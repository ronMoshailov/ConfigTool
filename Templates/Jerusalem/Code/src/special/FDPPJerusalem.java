...
class FDPPId1 {
	...
	// Byte 12 - Mode1 
	public int mode1;
	public boolean isControl;      // bit 0
	public boolean isOn;           // bit 1
	public boolean isFlash;        // bit 2
	public boolean isControlSigns; // bit 3
	public boolean isSignsOn;      // bit 4
	public boolean isCallAll;      // bit 5
	public boolean isReset;        // bit 6
	public boolean isSynced;       // bit 7
	// Byte 13 - Mode2
    ...
    public void setPulses(int pulsesBytesCount, int index) {
        ...
        for (nodeIterator = 1; nodeIterator <= TeilKnoten.getAnzahl(); nodeIterator++) {
            ...
            for (detIterator = 0; detIterator < currentNode.inputs.length; detIterator++) {
                if (!currentNode.inputs[detIterator].isLocalDetector()) {
                    currentNode.inputs[detIterator].setSimulation(pulses[counter] ? 1 : 0);
                    counter++;
                }
            }
        }
...
public class FDPPJerusalem implements Dvi35Base {
	...
	protected int connectionTimeout = 120 * Var.ONE_SEC;
	...
	private void resetCentralControl() {
	    ...
	    for (nodeIterator = 1; nodeIterator <= nodesCount; nodeIterator++) {
            currentNode = (Node)TeilKnoten.getTeilKnoten(nodeIterator);
            if (currentNode.getProgWunsch(currentNode, StgEbene.STG_ZENTRALE) != Vt.KEIN_PROGRAMMWUNSCH) {
    ...
    private void resetIncomingPulses() {
        ...
        for (nodeIterator = 1; nodeIterator <= nodesCount; nodeIterator++) {
            currentNode = (Node)TeilKnoten.getTeilKnoten(nodeIterator);
            for (detIndex = 0; detIndex < currentNode.inputs.length; detIndex++) {
                if (!currentNode.inputs[detIndex].isLocalDetector()) {
                    currentNode.inputs[detIndex].setSimulation(-1);
                }
            }
        }
    }
	...
	public void checkConnection() {
        ...
        connectionTimeout     = sbh.vtbib.dvi35.DVI35Modul.getId1(1003) * Var.ONE_SEC;
...
//		id1.printId1();
    ...
    public void setStatus() {
	    ...
	    if (id1.isReset) {
	        ...
	        fehlerhandler.FehlerHandler.writeMsg(fehlerhandler.FehlerHandler.errInfo, true, 1, "Reset TLC");
...
//                Var.tk1.fireState.resetAusgang();
    ...
    private void setCentralProgram(int progNumber) {
        for (nodeIndex = 1; nodeIndex <= TeilKnoten.getAnzahl(); nodeIndex++) {
            TeilKnoten.getTeilKnoten(nodeIndex).setProgWunsch(Vt.findProgByNum(progNumber, nodeIndex), StgEbene.STG_ZENTRALE);
        }   
    }
    ...
    private int getStatus2() {
        ...
        if (node_1.isWaitingForSy || (node_2 != null && node_2.isWaitingForSy))
            mask |= 0x10;
        if (node_1.isInMaintenance() || (node_2 != null && node_2.isInMaintenance()))
            mask |= 0x20;
...
//            mask |= 0x80;
    ...
    private boolean isInputSet(Input input) {
    	return input != null && input.belegt();
    }
    
    private int getInputStatus() {
        int mask = 0x00;
        
        if (isInputSet(Var.controller.upsActive))
            mask |= 0x01;
        if (isInputSet(Var.controller.upsWeak))
            mask |= 0x02;
        if (isInputSet(Var.controller.upsWarning))
            mask |= 0x04;
        if (isInputSet(Var.controller.upsFailure))
            mask |= 0x08;
        if (isInputSet(Var.controller.upsInBypass))
            mask |= 0x10;
        if (isInputSet(Var.controller.upsNoGridPower))
            mask |= 0x20;
        if (isInputSet(Var.controller.upsDoorClosed))
            mask |= 0x40;
        if (isInputSet(Var.controller.sitDoorOpen))
            mask |= 0x80;
	...
	public boolean operateSigns()
	{
	    if (id1.isControlSigns) {
	        if (id1.isSignsOn) {
	            Signs.signs_on.setAusgang();
	        } else {
	            Signs.signs_on.resetAusgang();
	        }
	        return true;
	    }
	    return false;
...
//        System.out.println("Starting Moves @" + byteIndex);
        ...
        DVI35Modul.setResIDx0(byteIndex++, 0xFF, connectionTimeout / Var.ONE_SEC);
    }
...
//            System.out.println();
            ...
            for (detIndex = 0; detIndex < node.demands.length; detIndex++) {
                if (node.demands[detIndex].isLocalDetector()) {
                    localDetsCount++;
                }
            }
            
            for (detIndex = 0; detIndex < node.demandExtensions.length; detIndex++) {
                if (node.demandExtensions[detIndex].isLocalDetector()) {
                    localDetsCount++;
                }
            }
            
            for (detIndex = 0; detIndex < node.extensions.length; detIndex++) {
                if (node.extensions[detIndex].isLocalDetector()) {
                    localDetsCount++;
                }
            }
            
            for (detIndex = 0; detIndex < node.queues.length; detIndex++) {
                if (node.queues[detIndex].isLocalDetector()) {
                    localDetsCount++;
                }
            }
            
            for (detIndex = 0; detIndex < node.pushButtons.length; detIndex++) {
                if (node.pushButtons[detIndex].isLocalDetector()) {
                    localDetsCount++;
                }
            }
            
            /*if (node.internals != null) {
                localDetsCount += node.internals.length;
            }
    ...
    private int setLocalPulses(int byteIndex) {
        ...
        for (nodeIndex = 0; nodeIndex < Var.controller.nodes.length; nodeIndex++) {
            Node node = Var.controller.nodes[nodeIndex];
            for (detIndex = 0; detIndex < node.outputs.length; detIndex++) {
                if (node.outputs[detIndex].isPulseToCenter()) {
                    localDetsCount++;
                }
            }
        }
        ...
        for (nodeIndex = 0; nodeIndex < Var.controller.nodes.length; nodeIndex++) {
            ...
            for (detIndex = 0; detIndex < node.outputs.length; detIndex++) {
                if (node.outputs[detIndex].isPulseToCenter()) {
                    if (node.outputs[detIndex].IsActive()) {
