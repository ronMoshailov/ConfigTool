...
//	protected int inputParamId;
	...
	protected boolean setOnFromParams;
	protected boolean setOffFromParams;
	protected boolean setFromCC;
	protected boolean isSetFromParamsEnabled;
	protected boolean isCallAllable;
	protected boolean isLocalDetector = true;
	 ...
	 */
	protected boolean isSetOnFromParameters() {
		if (Var.controller.getAppType().equals(AppType.Haifa)) {
			...
			if (!Var.controller.isPreemption()) {
				if (node.isOffline() || !node.isRegularProgram() || !this.isSetFromParamsEnabled) {
					return false;
				} else {
	...
	protected boolean isSetOffFromParameters() {
		if (Var.controller.getAppType().equals(AppType.Haifa)) {
			if (detectorId <= Var.controller.getMaximalDetectorId()) {
				return (ParSets.Det_Param[node.modeSet + detectorId - 1] > 0);
			} else {
			...
			if (!Var.controller.isPreemption()) {
				if (node.isOffline() || !node.isRegularProgram() || !this.isSetFromParamsEnabled) {
					...
					return ((ParametersJerusalem)Var.controller.dvi35Parameters).getDetectors(node, detectorId - 1) == Var.INPUT_DISABLED;
				}
			} else {
				return false;
			}
	...
	public Input setLocalDetector() {
		this.isLocalDetector = true;
		return this;
	}
	
	public Input setRemoteDetector() {
		this.isLocalDetector = false;
		return this;
	}
	
	public boolean isLocalDetector() {
		return this.isLocalDetector;
	}
	...
	protected boolean getSetFromCC() {
		if (Var.controller.getAppType().equals(AppType.Haifa)) {
			...
			return isCallAllable && Var.controller.dvi35.isCallAll();
		} else {
			return false;
		}
	}
	...
	public void UpdateState() {
		...
		setOffFromParams = isSetOffFromParameters();
		setFromCC = getSetFromCC();
		
		currentState = belegt() || (mapInput != null && mapInput.belegt());
		if (isSetFromParamsEnabled) {
			if (setOnFromParams)
				currentState = true;
			if (setOffFromParams)
				currentState = false;
