...
public class PhaseEQA1 extends Stage {
    protected Tk1 node;
     ...
     */
    public PhaseEQA1(Tk1 node, String name, int number, int length, boolean isStopInPolice, Move[] sgs) {
    ...
    public Phase phasenFunktion() {
        gaps            = !node.ps2.IsActive();
        isStageDoneFlag = isDone(gaps); 
        
        if (isStageDoneFlag) {
            if (node.d_3.IsActive()) {
                return switchStage(node.PhueEQA1_C);
            }
