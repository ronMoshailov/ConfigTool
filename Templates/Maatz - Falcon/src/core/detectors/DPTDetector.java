...
// ************************************************************************************************/
//import core.Node;
//import core.preemption.PreemptionH;
...
//	private PreemptionH _preemptionH;
//	
//	private Node _tk;
//	
...
//		return _preemptionH.dptData[_id][0];
//	}
//	
//	public int getDPTType() {
//		return _preemptionH.dptData[_id][1];
//	}
//	
//	public int getTimeToTarge() {
//		return _preemptionH.dptData[_id][3];
//	}
//
//	public DPTDetector(Node tk, int id, DPTDetector dpt, Sg sg, int dpt_type, int[] bus_lines, int delta)
//	{
...
//		_preemptionH = (PreemptionH)tk.preemption;
//		_preemptionH.dptData[_id][0] = sg.getNr();
//		_preemptionH.dptData[_id][1] = dpt_type;
//		_preemptionH.dptData[_id][2] = _id;
//		_isNotLogical = false;
//	}
//
//	/**
...
//	 * @param dpt_type
//	 */
//	public DPTDetector(Node tk, PreemptionH prm, int num_of_channels, int id, int logical_det_channel, Sg sg, String[] sg_name, int dpt_type, int[] bus_lines, int delta)
//	{
//		int i, j;
//		_tk = tk;
//		if (sg_name.length > 1) {
...
//	}
//	
//	public DPTDetector(Node tk, PreemptionH prm, int num_of_channels, int id, int logical_det_channel, Sg sg, String[] sg_name, int dpt_type, int[] bus_lines, int delta, Ausgang[] outputs, Ausgang[] okOutputs)
//	{
//		int i, j;
//		_tk = tk;
//		if (sg_name.length > 1) {
...
//	}
//	
//	public DPTDetector(Node tk, PreemptionH prm, int num_of_channels, int id, int logical_det_channel, String[] sg_name, int dpt_type, int[] bus_lines, int delta)
//	{
//		int i, j;
//		_tk = tk;
//		if (sg_name.length > 1) {
...
//	}
//	
//	public DPTDetector(Node tk, PreemptionH prm, int num_of_channels, int id, int logical_det_channel, String[] sg_name, int dpt_type, int[] bus_lines, int delta, Ausgang[] outputs, Ausgang[] okOutputs)
//	{
//		int i, j;
//		_tk = tk;
//		if (sg_name.length > 1) {
...
//		
//		if (_id >= 0 &&
//	        _id < _preemptionH.dptData.length && // index of detector does not exceed maximum number of dettectors
//	        _preemptionH.dptData[_id][0] != PreemptionH.NONE) // detector was defined
//		{
//			if (_num_of_channels == 1) {
//				return (channels[0].getPosFlanken() > 0) || _isManualDemand;
//			} else if (_num_of_channels == 2) {
...
//        for (int i = 0; i < _preemptionH.BrtsPerMove; i++ ) {
//            if (_preemptionH.atCounters[index][i] == PreemptionH.NONE) { // first empty space (next bus will be put here)
//            	_preemptionH.atCounters[index][i] = value; // set the length of the far detector
//            	_preemptionH.atTypes[index][i] = detectorNumber;
//                return;
//            }
//        }
//    }
...
//        int closestIndex = PreemptionH.NONE;
//        int diff = 1000*Var.ONE_SEC;
//        // find closest to value
//        for (int i = 0; i < _preemptionH.BrtsPerMove; i++) {
//            if ((_preemptionH.atCounters[index][i] != PreemptionH.NONE) && (detectorNumber > _preemptionH.atTypes[index][i]))
//            { // first detected bus which detector index is lower than current detected dpt
//                int distance = _preemptionH.atCounters[index][i] - value;
//                if (distance < 0) {
//                    distance = -distance;
//                }
//                if (distance < diff) {
...
//        }
//
//        if (closestIndex == PreemptionH.NONE) {
//            startCounter(index, value, detectorNumber);
//        } else {
//            _preemptionH.atCounters[index][closestIndex] = value;
//            _preemptionH.atTypes[index][closestIndex] = detectorNumber;
//        }
//    }
//
//    private void resetCounter(int index) {
//        int closestValue = PreemptionH.NONE;
//        int closestIndex = PreemptionH.NONE;
//
//        for (int i = 0; i < _preemptionH.BrtsPerMove; i++) {
//            if (_preemptionH.atCounters[ index ][ i ] < closestValue) {
//                closestValue = _preemptionH.atCounters[index][i];
//                closestIndex = i;
//            }
//        }
//
//        if (closestIndex != PreemptionH.NONE) {
//            _preemptionH.atCounters[index][closestIndex] = PreemptionH.NONE;
//            _preemptionH.atTypes[index][closestIndex] = PreemptionH.NONE;
//            _preemptionH.dpt2Timeout[index][closestIndex] = 0;
//            _preemptionH.resetMemBRT(index);
//        }
//    }
//
...
//		
//        if (_id >= 0 &&
//        	_id < _preemptionH.dptData.length && // index of detector does not exceed maximum number of detectors
//        	_preemptionH.dptData[_id][0] != PreemptionH.NONE) // detector was defined
//        {
//            int moveIndex = _preemptionH.getMoveIndex( _preemptionH.dptData[_id][0]);
//
...
//            {
//            	// 
//                int length = _preemptionH.getBrtDetectorTime(_preemptionH.dptData[_id][2]);
//                if (_preemptionH.dptData[_id][1] == 1) // far detector activated
//                    startCounter(moveIndex, length, 1 );
//
//                if (_preemptionH.dptData[_id][1] == 2) // near detector activated
//                    updateCounter(moveIndex, length, 2);
//
//                if (_preemptionH.dptData[_id][1] == 3) // stopline detector activated
//                    updateCounter(moveIndex, length, 3);
//
//                if (_preemptionH.dptData[_id][1] == 4) // cancellation detector activated
