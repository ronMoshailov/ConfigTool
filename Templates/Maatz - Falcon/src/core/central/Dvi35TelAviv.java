...
public class Dvi35TelAviv extends iCentral {
    class Dvi35Id1TA {
        ...
        public boolean statusBit4;  // Bit 4
        ...
        public void setStatus(int status) {
            ...
            statusBit4      = ((this.status >> 4) & 1) != 0 ? true : false;
	...
	public void getId1()
	{
		...
		for (nodeIterator = 1; nodeIterator <= nodesCount; nodeIterator++) {
			...
			if (isOperateInCentralDark) {
				...
				if (currentNode.isMapOnlyProgram() || currentNode.isMaintenanceProgramRequest()) {
...
//		}
		...
		if (node_1.getAktStgEbene() == StgEbene.STG_NUM_ZENTRALE || (node_1.isMapOnlyProgram() && isOperateInCentral)) {
			statusByteMask |= 0x20;
		}

		// Manipulating byte 12 bit 6 (Maintenance / Map Only Mode) of the remote frame to control center
		if (node_1.isMapOnlyProgram()) {
			statusByteMask |= 0x40;
		}
		...
		if ((node_1.getAktProg().getNummer() >= 17 && node_1.getAktProg().getNummer() <= 32) || 
			(node_1.isMapOnlyProgram() && (node_1.getAktProg().getNummer() - SpecialProgram.MaintenanceFirst.getProgramNumber() + 1 >= 17 && node_1.getAktProg().getNummer() - SpecialProgram.MaintenanceFirst.getProgramNumber() + 1 <= 32))) {
			statusByteMask |= 0x80;
		}

		if (node_1.getAktProg().getNummer() == node_1.all_dark.getNummer()) {
			DVI35Modul.setResIDx0(1, 0xFF, 0);
		} else if (node_1.isMapOnlyProgram()) {
			if (node_1.isMapOnlyProgram() && (node_1.getAktProg().getNummer() - SpecialProgram.MaintenanceFirst.getProgramNumber() + 1 >= 17 && node_1.getAktProg().getNummer() - SpecialProgram.MaintenanceFirst.getProgramNumber() + 1 <= 32)) {
		...
		if (Var.controller.ups.upsActive != null && Var.controller.ups.upsActive.belegt())
			specialStatusByteMask |= 0x01;
		if (Var.controller.ups.upsFailure != null && Var.controller.ups.upsFailure.belegt())
			specialStatusByteMask |= 0x02;
		if (Var.controller.ups.upsDoorClosed != null && !Var.controller.ups.upsDoorClosed.belegt())
			specialStatusByteMask |= 0x04;
		if (Var.controller.isDoorOpen())
			specialStatusByteMask |= 0x08;
		if (!Var.controller.policePanel.Door_Closed.eingangGesetzt())
