...
public class PhaseD extends Stage {
	private Tk1 node;
	 ...
	 */
	public PhaseD(Tk1 node, String name, int number, int length, boolean isStopInPolice, Move[] sgs) {
	 ...
	 * @param sgs - list of moves that must open in this stage
	 */
	public PhaseD(Tk1 node, String name, int numberr, int length, int sp, boolean isStopInPolice, Move[] sgs) {
	...
	public Phase phasenFunktion() {
		...
		if (getPhasenZeit() >= node.lenPhue + getPhLenMS())
		{
			...
			if (isStageDoneFlag) 
			{
				if(node.d_6.IsActive()|| Var.hand_active)
					return startNextPhase(node.PhueD_E);
				return startNextPhase(node.PhueD_A);
			}
		}
