...
class DataManager:
    ...
    def update_images(self, table_dict):
        ...
        for image_name, table in table_dict.items():
            ...
            for row_num in range(table.rowCount()):
                ...
                checkbox = table.cellWidget(row_num, 1).findChild(QCheckBox)
            ...
            for image in self.images:
                if image.image_name == image_name:
                    ...
                    is_found = True
            if not is_found:
                self.images.append(Image(image_name, checked_moves, int(skeleton_time)))
    ...
    def update_inter_stage(self, table_wrap_list):
        ...
        for wrap in table_wrap_list:
            ...
            img_in = wrap.img_in
            tbl = wrap.table
            inter_stage_len = wrap.len_textbox.text()

            new_inter_stage = InterStage(img_out, img_in, inter_stage_len)
    ...
    def remove_inter_stage(self, img_out, img_in):
        for inter_stage in self.inter_stage:
            if inter_stage.image_out == img_out and inter_stage.image_in == img_in:
                self.inter_stage.remove(inter_stage)
                return True, "המחיקה הצליחה"
        return False, "המעבר לא נמצא"
